...
export default function NavLinks({ user, isMobile = false, setIsOpen, logoutAction }: { user: any, isMobile?: boolean, setIsOpen?: (v: boolean) => void, logoutAction?: any }) {
  ...
  if (isMobile) {
    return (
      <>
        <Link onClick={() => setIsOpen?.(false)} href="/" className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-white hover:text-sky-400 transition-colors uppercase">
          Return to Homepage
        </Link>
        {user ? (
          <>
            <Link onClick={() => setIsOpen?.(false)} href="/admin" className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-white hover:text-sky-400 transition-colors uppercase">
              Admin Dashboard
            </Link>
            <form action={logoutAction} className="inline">
              <button type="submit" onClick={() => setIsOpen?.(false)} className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-white hover:text-sky-400 transition-colors uppercase text-left w-full">
                Logout
              </button>
            </form>
          </>
        ) : (
          <Link onClick={() => setIsOpen?.(false)} href="/login" className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-white hover:text-sky-400 transition-colors uppercase">
            Login
          </Link>
        )}
        ...
        {links.map((link) => (
          <Link 
            ...
            href={link.href} 
            className={`text-2xl sm:text-3xl md:text-4xl font-serif font-bold transition-colors uppercase ${isActive(link.href) ? 'text-sky-400' : 'text-white hover:text-sky-400'}`}
          >
            {link.label}
          </Link>
        ))}
