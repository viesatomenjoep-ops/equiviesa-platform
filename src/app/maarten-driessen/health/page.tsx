...
export default function HealthPage() {
  ...
  useEffect(() => {
    async function fetchLogs() {
      ...
      const { data, error } = await supabase.from('health_logs').select('*, horses(name)').order('date', { ascending: false });
      if (!error && data) {
        setLogs(data);
      }
      setLoading(false);
    ...
    <div className="min-h-screen bg-gray-50 text-amber-900 pb-20">
      ...
      <section className="pt-32 pb-12 px-6 bg-slate-900 text-white rounded-b-[3rem]">
        <div className="max-w-6xl mx-auto text-center mt-8">
          <h1 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight text-white">Medisch Dossier</h1>
          <p className="text-xl text-slate-300 mb-8 font-light">
            Overzicht van vaccinaties, ontworming en dierenarts bezoeken.
          </p>
        </div>
      </section>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {logs.map(log => (
                <div key={log.id} className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100 flex flex-col">
                  <div className="flex justify-between items-start mb-4">
                    <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-xs font-bold uppercase">{log.type}</span>
                  </div>
                  <h3 className="text-xl font-bold text-slate-900 mb-1">{log.horses?.name || 'Algemeen Medisch Record'}</h3>
                  <p className="text-slate-600 mb-4">{log.description}</p>
                  
                  <div className="space-y-2 mt-auto">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Calendar className="w-4 h-4" /> Datum: {new Date(log.date).toLocaleDateString()}
                    </div>
                  </div>
                </div>
              ))}
              {logs.length === 0 && (
                <div className="col-span-2 text-center p-12 bg-white rounded-3xl border border-gray-100">
                  <p className="text-lg text-slate-500">Geen medische logs gevonden.</p>
                </div>
              )}
            </div>
          )}
